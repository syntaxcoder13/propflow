...
exports.parseIndianPrice = parseIndianPrice;
exports.parseAreaSqFt = parseAreaSqFt;
exports.runMaharashtraScraper = runMaharashtraScraper;
...
function parseIndianPrice(priceStr) {
    if (!priceStr)
        return { priceText: "₹85 L", numericPrice: 8500000 };
...
function parseAreaSqFt(areaStr) {
    if (!areaStr)
        return 750;
...
// ==========================================
const MAHARASHTRA_SEED_LISTINGS = [
    {
        id: "mah-mum-101",
...
// SCRAPER ENGINE
// ==========================================
async function scrapeTargetUrl(target) {
    console.log(`[Scraper] Requesting target URL: ${target.url} (${target.city})...`);
    const listings = [];
    try {
        const response = await axios_1.default.get(target.url, {
            headers: HEADERS,
            timeout: 10000,
        });
        ...
        if (nextDataScript) {
            try {
                ...
                const rawItems = parsedData?.props?.pageProps?.initialState?.search?.listings ||
    ...
    if (listings.length === 0) {
        ...
        const citySeed = MAHARASHTRA_SEED_LISTINGS.filter((item) => item.location.city.toLowerCase() === target.city.toLowerCase());
...
// MAIN RUNNER
// ==========================================
async function runMaharashtraScraper() {
    console.log("==================================================");
    ...
    const outputPath = path_1.default.join(dataDir, "maharashtra_properties.json");
    fs_1.default.writeFileSync(outputPath, JSON.stringify(finalResults, null, 2), "utf-8");
    console.log("==================================================");
